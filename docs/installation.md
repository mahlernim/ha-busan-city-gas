...
- 선택한 도시가스 공급사 홈페이지에서 로그인할 수 있는 회원 아이디·비밀번호
...
1. **설정 → 기기 및 서비스 → 통합 추가 → SK E&S 도시가스**를 선택합니다.
2. 청구서에 표시된 공급사를 선택하고 해당 공급사의 **회원 아이디·비밀번호**를 입력합니다.
3. 계약이 여러 개라면 사용할 계약을 선택합니다. 하나라면 다음 단계로 넘어갑니다.
4. 계약의 주택용 요금 유형을 선택합니다. 코원에너지서비스는 서울 또는 경기 지역도 선택합니다.
5. 가스센서가 있다면 원본 누적 센서를 선택합니다. 없다면 건너뜁니다.
6. 계량기에 보이는 숫자를 선택적으로 입력하고 알림·제출 정책을 설정합니다. 자동 제출은 처음에는 꺼두세요.
7. 요약을 확인하고 완료합니다. 사이드바에 **SK E&S 도시가스**가 나타납니다.
